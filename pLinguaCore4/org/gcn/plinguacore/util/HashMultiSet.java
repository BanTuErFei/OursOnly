...
public class HashMultiSet<E> implements MultiSet<E> {
	...
	*/
	public boolean replaceString(Object find, 
		...
		System.out.println("HashMultiSet :: replaceString");
		System.out.println("In replaceString, object: " + find.toString());
		System.out.println("In replaceString, replaceWith: " + replaceWith.getClass());
		System.out.println("long executions " + executions);

		//Need to be an object first. Just typecast later
		...
		findString = findString.substring(1,findString.length()-1);
		System.out.println("findString " + findString);

		//Created a duplicate and cleared the original
		duplicate = ((LinkedHashMap)entrySet.map).clone();
		//System.out.println("duplicate " + entrySet.map.keySet().getClass());
		duplicateArray = ((LinkedHashMap)duplicate).keySet().toArray();
		entrySet.map.clear();
		

		int size = duplicateArray.length;
		System.out.println("duplicateArray " + duplicateArray.toString());
		for(int j=0; j<size; j++){
			System.out.println(duplicateArray[j]);
		}
		System.out.println("size = " + size);
		int ctr = size, index;
		long execMembrane, execRemaining, multiplicity, curr; 

		//Iterator iteratorReplaceWith = replaceWith.iterator();
		//System.out.println("replaceWith " + iteratorReplaceWith.next().getClass());
				
		while(executions > 0){	//this pass is per membrane
			Random rnd= new Random();
			index = rnd.nextInt(ctr);
			//System.out.println("index = " + index);
		
			if(duplicateArray[index].toString().contains(findString)){
				Iterator iteratorReplaceWith = replaceWith.iterator();
				System.out.println("replaceWith count = " + replaceWith.size());
				//System.out.println("duplicate values " + ((LinkedHashMap)duplicate).get(((String[])duplicateArray)[index]).getClass());
				
				execMembrane = (Long)(((LinkedHashMap)duplicate).get(duplicateArray[index]));
				System.out.println("execMembrane " + execMembrane);

				//if the multiplicity of membrane is greater than execution needed
				//then subtract and only the multiplicity needed will be removed
				if(execMembrane > executions){
					multiplicity = executions;
					execRemaining = execMembrane - executions;
					executions = 0;
				}
				else{
					executions = executions - execMembrane;
					multiplicity = execMembrane;
					execRemaining = 0;
				}

				System.out.println("execRemaining = " + execRemaining);
				System.out.println("multiplicity = " + multiplicity);

				while(iteratorReplaceWith.hasNext()){
					String replaceWithString = (String)iteratorReplaceWith.next();
					replaceWithString = replaceWithString.substring(1,replaceWithString.length()-1);
					String duplicateArrayReplaced = ((String)duplicateArray[index]).replaceAll(findString,replaceWithString);
					System.out.println("duplicateArrayReplaced = " + duplicateArrayReplaced);
					
					/*if(entrySet.map.containsKey((E)duplicateArrayReplaced))
						curr = entrySet.map.get((E)duplicateArrayReplaced) + multiplicity;
					else
						curr = multiplicity;*/

					entrySet.map.put((E)duplicateArrayReplaced,multiplicity);
				}

				/*Iterator tempIter = entrySet.iterator();
				while(tempIter.hasNext()){
					System.out.println("tempIter " + tempIter.next().toString());
				}*/

				if(execRemaining > 0){
					if(entrySet.map.containsKey((E)duplicateArray[index]))
						curr = entrySet.map.get((E)duplicateArray[index]) + execRemaining;
					else
						curr = execRemaining;

					entrySet.map.put((E)duplicateArray[index],curr);
				}

				String temp = (String)duplicateArray[ctr-1];
				duplicateArray[ctr-1] = duplicateArray[index];
				duplicateArray[index] = temp;
		...
		//remaining in duplicateArray[] return back to entrySet
		for(int i=0; i<ctr; i++){
			entrySet.map.put((E)duplicateArray[i],(Long)(((LinkedHashMap)duplicate).get(duplicateArray[i])));
		}


		System.out.println("LAMAN NG ENTRYSET");
		Iterator tempIter = entrySet.iterator();
		while(tempIter.hasNext()){
				System.out.println("tempIter " + tempIter.next().toString());
		}
	...
	public long count(Object object) {
		//System.out.println("count in hashmultiset " + object.toString());
	...
	public long countInMembrane(Object object){
		int cnt = 0;
		...
		if(object instanceof String){
			...
			if(obj.startsWith("<")){
				...
				while(iteratorMultiset.hasNext()){
					...
					if(iterInstance instanceof String){
		...
		if (contains(object)){
			//System.out.println("HashMultiSet in Membrane:: " + object.toString() + " :: count :: " +  (cnt + entrySet.map.get(object).longValue()) );	
			return cnt += entrySet.map.get(object).longValue();	//JM: what is this longvalue?
		}
		//System.out.println("HashMultiSet inMembrane:: " + object.toString() + " :: count :: " + cnt);	
		return cnt;	
	}
	...
	public long countSubSets(Collection<?> objects) {	//JM: what does this do anyway??
		...
		while (it.hasNext()) {
			Object o = it.next();	//o is the leftHandRule
			//System.out.println("countSubSets :: " + o.toString());
			//System.out.println("HashMultiSet: countSubSets = " + o.toString());
			//long m1 = count(o);	//count the number of objects in the membrane
			long m1 = countInMembrane(o);
			long m2 = multiSet.count(o);	//count the number of objects in the rule
	...
	public String toString() {
		//System.out.println("toString in HashMultiSet");
	...
	@Override
	public boolean equals(Object obj) {
		//System.out.println("hashmultiset equals");
		if (this == obj)
			return true;
		if (obj == null)
			return false;
		...
		//System.out.println(((HashMultiSet)other).entrySet.map.keySet().iterator().next().getClass() + " " + this.getClass());
