	...
	protected boolean executeSafeString(ChangeableMembrane membrane, 
		...
		*/
		...
		if(outerClm.isSkinMembrane()){			
			...
			if(isEvolution()){
				//System.out.println("membrane.getMultiSet " + (membrane.getMultiSet()).getClass());
				//System.out.println("leftHandRule.getMultiSet " + getLeftHandRule().getOuterRuleMembrane().getMultiSet().toString());
				
				replaceMultiSet(membrane.getMultiSet(),getLeftHandRule().getOuterRuleMembrane().getMultiSet(),getRightHandRule().getOuterRuleMembrane().getMultiSet(),executions); 
